...
def get_chat_status(
    ...
    if 'Item' not in result:
        logger.error(
            '[get_chat_status] Empty response for {}'.format(chat_hash))
        return -1

    if len(result['Item']) == 0:
        logger.error(
            '[get_chat_status] Item array as Item {}'.format(chat_hash))
        return -1
...
def get_user_lang(
    ...
    try:
        ...
        logger.error(
            '[get_user_lang] Unable to return Language'
            f'{str(error)}')
        return None
